...
export type AreaChartDatum = { label: string; value: number };
...
export function AreaChart({ data, title, description }: AreaChartProps) {
  const gradientId = `amber-area-${useId().replace(/:/g, "")}`;
    ...
    return (
      ...
      >
        ...
        <p className="mt-1 max-w-sm text-sm text-muted-foreground">
          Add time series or category values to render this area chart.
        </p>
      </figure>
    );
  }
